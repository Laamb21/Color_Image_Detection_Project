'''
Modifying code to select single file path rather than manually selecting TIFF and JPG directories
'''
...
# Main function to process TIFF and JPEG images in subdirectories
def process_images_in_subdirectories(base_dir, output_dir):
    for root, dirs, files in os.walk(base_dir):
        # Separate TIFF and JPEG files in each directory
        tiff_files = [f for f in files if f.lower().endswith('.tif')]
        jpeg_files = {f.replace("10", "11").replace("20", "21"): os.path.join(root, f) for f in files if f.lower().endswith('.jpg')}
        ...
        for tiff_filename in tiff_files:
            if tiff_filename.startswith("10") or tiff_filename.startswith("20"):
                ...
                jpeg_path = jpeg_files.get(color_filename)
...
def run_in_thread(base_dir, output_dir):
    thread = threading.Thread(target=process_images_in_subdirectories, args=(base_dir, output_dir))
    thread.start()
...
def create_gui():
    ...
    base_button = Button(root, text="Select Base Directory", command=lambda: select_directory(base_label))
    ...
    run_button = Button(root, text="Run", command=lambda: run_in_thread(base_label.cget("text"), output_label.cget("text")))
    run_button.pack(pady=20)

    # Text widget for displaying output (moved below the Run button, and made significantly larger)
    output_text = Text(root, height=30, wrap='word')  # Increased height to 30 for more lines
    output_text.pack(pady=10, fill='x', expand=True)
